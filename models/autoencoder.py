...
class Autoencoder(BaseModel):
    ...
    def _add_variational_loss(self):
        if self.adversarial_training:
            self.seg_loss_op = tf.reduce_mean(
                self.xentropy_loss_op + self.adv_lambda*self.l_bce_fake_one + self.KLD)
        else:
            self.seg_loss_op = tf.reduce_mean(self.xentropy_loss_op + self.KLD)


    """ Add variational mode to test ops

    The new test op should sample from normal distribution and compute y_hat
    """
    ...
    def model(self, input_op, reuse=False, training=True):
        ...
        if self.variational:
            self.mu = slim.fully_connected(self.zed, self.zed_dim, scope='mu', reuse=reuse, activation_fn=None)
            self.logvar = slim.fully_connected(self.zed, self.zed_dim, scope='logvar', reuse=reuse, activation_fn=None)
...
#         # if self.bayesian:
#         #     net = slim.dropout(net, scope='drop1')
#         conv3_1 = slim.convolution2d(conv3_0, self.n_kernels*4, 3, 1, padding='mmmm', s cope='conv3_1',reuse=reuse)
#         conv3_2 = slim.convolution2d(conv3_1, self.n_kernels*4, 3, 1, padding='SAME', scope='conv3_2',reuse=reuse)
#         conv3_2 = slim.batch_norm(conv3_2, scope='bn3', reuse=reuse, is_training=training)
#
#         zed_3 = slim.flatten(conv3_2, scope='flat2')
...
#         zed_upsample = slim.convolution2d_transpose(zed, self.n_kernels*4, 3, 2, padding='VALID', scope='zed_upsample', reuse=reuse)
#
#         deconv1_0 = slim.convolution2d_transpose(zed_upsample, self.n_kernels*2, 5, 2, padding='VALID', scope='deconv1_0', reuse=reuse)
#         deconv1_0 = slim.batch_norm(deconv1_0, scope='bn5', reuse=reuse, is_training=training)
#         # if self.bayesian:
...
#         deconv2_1 = slim.convolution2d_transpose(deconv2_0, self.n_kernels, 5, 2, padding='VALID', scope='deconv2_1', reuse=reuse)
#         deconv2_1 = slim.batch_norm(deconv2_1, scope='bn7', reuse=reuse, is_training=training)
#         deconv2_1_resize = tf.image.resize_bilinear(deconv2_1, [self.x_dim//2, self.y_dim//2])
#
#         deconv3_0 = slim.convolution2d_transpose(deconv2_1_resize, self.n_classes, 2, 2, padding='VALID', scope='deconv3_0', reuse=reuse)
#         deconv3_0 = slim.batch_norm(deconv3_0, scope='bn8', reuse=reuse, is_training=training)
#         deconv3_0_resize = tf.image.resize_image_with_crop_or_pad(deconv3_0, self.x_dim, self.y_dim) ## Take care of off-by-one
#
#         deconv_out = slim.convolution2d(deconv3_0_resize, self.n_classes, 3, 1, padding='SAME', scope='conv_out', reuse=reuse,
#             activation_fn=None)
#
#
#         print '\tdecode_zed_reshape', zed.get_shape()
#         print '\tzed_upsample', zed_upsample.get_shape()
#         print '\tdeconv1_0', deconv1_0.get_shape()
#         print '\tdeconv2_0', deconv2_0.get_shape()
#         print '\tdeconv2_1', deconv2_1.get_shape()
#         print '\tresize', deconv2_1_resize.get_shape()
#         print '\tdeconv3_0', deconv3_0.get_shape()
#         print '\tforce_resize', deconv3_0_resize.get_shape()
#         print '\tconv_output', deconv_out.get_shape()
#
#         return net
